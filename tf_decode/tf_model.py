...
def process_data(a, transpose_input=False):
  batch_size = len(a)
  if transpose_input:
    a = np.reshape(a, [batch_size] + FEATURE_INPUT_SHAPE)
    a = np.transpose(a, [0,1,3,2])
    a = np.reshape(a, [batch_size, -1])
  return a
    
def inference(images, hidden1_units=2048, hidden2_units=2048, hidden3_units=2048):
  ...
  """
  x_image = tf.reshape(images, [-1,11,40,3])
  # conv1
  with tf.variable_scope("conv"):
    W_conv1 = weight_variable([11, 5, 3, 180])
    ...
    W_conv3 = weight_variable([1, 5, 180, 180])
  ...
  with tf.variable_scope("hidden1", reuse=True):
    ...
    h1_input_size = 1 * 3 * 180
    h_pool2_flat = tf.reshape(h_conv3, [-1, h1_input_size])
    weights = tf.Variable(
    ...
    h_fc1_drop = tf.nn.dropout(hidden1, 0.5)
  # Hidden 2
